...
| `karet-markdown` | engine | — | markdown (plus a curated embedded-HTML subset, no layout engine) parse → wrap → render model, with source-line anchors for scroll sync and image rows reserved through a consumer's `ImageSizer`; `highlight` colours code fences (`lang-common`/`all-languages` bundle the grammars), `view` paints ratatui (incl. a scrollable `MarkdownView`) |
...
| `karet-seam` | engine | — | a repository's **seams** as a queryable index: package discovery (Cargo, Python, Node, Swift, Gradle) → containment tree with a root per package, regrouped so members sit under the type they belong to whatever syntax declared them, per-lens facets (api/substitution/variation/boundary/hazard), edges, `cfg` configurations, and a predicate query language; builds in parallel (rayon) and emits a replayable per-file `FileContribution` so a caller can cache it; `lang-rust`/`lang-python`/`lang-typescript`/`lang-swift`/`lang-kotlin` mappings |
