...
| `karet-seam` | engine | — | a repository's **seams** as a queryable index: package discovery (Cargo, Python, Node, Swift, Gradle) → containment tree with a root per package, regrouped so members sit under the type they belong to whatever syntax declared them, per-lens facets (api/substitution/variation/boundary/hazard), edges, `cfg` configurations, and a predicate query language; `lang-rust`/`lang-python`/`lang-typescript`/`lang-swift`/`lang-kotlin` mappings |
