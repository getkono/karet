...
| Jupyter notebook | nbformat-4 converted to a standalone rendered Markdown preview by [`karet-notebook`](../crates/karet-notebook) — code cells fenced in the notebook's language, outputs text-first (images as placeholders), tracebacks ANSI-stripped | `ipynb` |
