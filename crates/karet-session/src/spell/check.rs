//! The pure spell-check pass: text plus a token model in, diagnostics out.
//!
//! Kept apart from the debounce worker in [`super`] because two callers share it —
//! the per-document worker, which checks one open buffer at a time and wants
//! suggestions ready for the quick-fix menu, and the workspace scan, which checks
//! thousands of files off disk and wants none (spellbook's suggester is the dominant
//! per-word cost and a results list does not show them). Nothing here touches the
//! filesystem, a channel, or a clock.

use std::collections::HashSet;
use std::sync::OnceLock;

use karet_core::Diagnostic;
use karet_core::LineCol;
use karet_core::Range;
use karet_core::Severity;
use karet_core::StandardToken;
use karet_core::TokenId;
use karet_syntax::Highlights;
use spellbook::Dictionary;

use super::context::is_non_prose_string;
use crate::api::SpellingLanguage;
use crate::config::schema::Spellcheck;

const SOFTWARE_TERMS: &str = include_str!("../../assets/software-terms.txt");

/// The `source` every spell diagnostic carries, so a consumer can tell the spell
/// layer apart from an LSP's.
pub(crate) const SPELL_SOURCE: &str = "karet-spell";

/// One immutable unit of text to check, with everything the scope rules need.
pub(crate) struct SpellInput<'a> {
    /// The full text being checked.
    pub text: &'a str,
    /// The display language name (`"Markdown"`, `"Rust"`, …), used only to decide
    /// whether this is a prose document.
    pub language: Option<&'static str>,
    /// The stable per-language settings selector (`karet-filetype`'s
    /// `config_selector`) — the machine key for language-aware behavior. The
    /// display `language` above is presentation-only and never dispatched on.
    pub language_selector: Option<&'static str>,
    /// Which dictionary to check against.
    pub spelling_language: SpellingLanguage,
    /// The token model deciding what counts as prose. An empty `Highlights` still
    /// checks a prose document; a source file simply contributes nothing.
    pub highlights: &'a Highlights,
    /// Line ranges covered by syntax errors, which pause identifier linting.
    pub syntax_error_lines: &'a [(u32, u32)],
    /// The resolved spell-check settings.
    pub settings: &'a Spellcheck,
    /// Compute replacement suggestions for each unknown word and append them to
    /// the diagnostic message. Off for the workspace scan.
    pub suggest: bool,
}

/// Check `input` against `dictionary`, returning one warning per unknown word.
pub(crate) fn check(input: &SpellInput<'_>, dictionary: &Dictionary) -> Vec<Diagnostic> {
    let custom: HashSet<String> = input
        .settings
        .words
        .iter()
        .map(|word| word.to_lowercase())
        .collect();
    let document = is_prose_document(input.language) && input.settings.documents;
    let line_index = LineIndex::new(input.text);
    let mut diagnostics = Vec::new();
    for (start, end, word) in words(input.text) {
        let Some(scope) = spell_scope(input, start, document) else {
            continue;
        };
        let candidates = if scope == SpellScope::Identifier {
            identifier_words(start, word)
        } else {
            vec![(start, end, word)]
        };
        for (start, end, word) in candidates {
            let normalized = word.to_lowercase();
            if (scope != SpellScope::Identifier && should_skip_context(input.text, start, end))
                || custom.contains(&normalized)
                || software_terms().contains(normalized.as_str())
                || dictionary.check(word)
                || (scope != SpellScope::Identifier && should_skip_proper_name(word))
            {
                continue;
            }
            push_diagnostic(
                &mut diagnostics,
                &line_index,
                start,
                end,
                word,
                input,
                dictionary,
            );
        }
    }
    diagnostics
}

fn software_terms() -> &'static HashSet<&'static str> {
    static TERMS: OnceLock<HashSet<&'static str>> = OnceLock::new();
    TERMS.get_or_init(|| {
        SOFTWARE_TERMS
            .lines()
            .map(str::trim)
            .filter(|line| !line.is_empty() && !line.starts_with('#'))
            .collect()
    })
}

fn push_diagnostic(
    diagnostics: &mut Vec<Diagnostic>,
    line_index: &LineIndex<'_>,
    start: usize,
    end: usize,
    word: &str,
    input: &SpellInput<'_>,
    dictionary: &Dictionary,
) {
    let mut suggestions = Vec::new();
    if input.suggest {
        dictionary
            .suggester()
            .with_ngram_suggestions(false)
            .suggest(word, &mut suggestions);
        suggestions.truncate(3);
    }
    let message = if suggestions.is_empty() {
        format!("Unknown word “{word}”")
    } else {
        format!("Unknown word “{word}”; try {}", suggestions.join(", "))
    };
    diagnostics.push(Diagnostic {
        range: Range {
            start: line_index.position(start),
            end: line_index.position(end),
        },
        severity: Severity::Warning,
        message,
        source: Some(SPELL_SOURCE.to_owned()),
        code: Some(input.spelling_language.locale().to_owned()),
        tags: Vec::new(),
        related: Vec::new(),
    });
}

fn is_prose_document(language: Option<&str>) -> bool {
    language.is_some_and(|language| {
        matches!(
            language.to_ascii_lowercase().as_str(),
            "markdown" | "plain text" | "asciidoc" | "restructuredtext" | "tex"
        )
    })
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum SpellScope {
    Prose,
    Identifier,
}

fn spell_scope(input: &SpellInput<'_>, start: usize, document: bool) -> Option<SpellScope> {
    let token = token_at(input.highlights, start);
    if document {
        return (!matches!(
            token,
            Some(token)
                if token == StandardToken::MarkupRaw.id()
                    || token == StandardToken::MarkupLink.id()
                    || is_code_token(token)
        ))
        .then_some(SpellScope::Prose);
    }
    if input.settings.comments
        && token.is_some_and(|token| is_comment_token(token) || is_markup_prose_token(token))
    {
        return Some(SpellScope::Prose);
    }
    if input.settings.strings
        && token == Some(TokenId::STRING)
        && !is_non_prose_string(input.text, input.language_selector, input.highlights, start)
    {
        return Some(SpellScope::Prose);
    }
    (input.settings.identifiers
        && input.syntax_error_lines.is_empty()
        && token.is_some_and(is_identifier_token))
    .then_some(SpellScope::Identifier)
}

fn token_at(highlights: &Highlights, byte: usize) -> Option<TokenId> {
    let spans = highlights.all();
    let before = spans.partition_point(|span| span.span.start.0 <= byte);
    spans[..before]
        .iter()
        .rev()
        .find(|span| byte < span.span.end.0)
        .map(|span| span.token)
}

fn is_comment_token(token: TokenId) -> bool {
    token == TokenId::COMMENT
        || token == StandardToken::CommentDoc.id()
        || token == StandardToken::CommentMark.id()
}

fn is_markup_prose_token(token: TokenId) -> bool {
    matches!(
        token,
        token if token == StandardToken::MarkupHeading.id()
            || token == StandardToken::MarkupBold.id()
            || token == StandardToken::MarkupItalic.id()
            || token == StandardToken::MarkupQuote.id()
            || token == StandardToken::MarkupStrikethrough.id()
    )
}

fn is_code_token(token: TokenId) -> bool {
    token == TokenId::KEYWORD
        || token == TokenId::FUNCTION
        || token == TokenId::TYPE
        || token == TokenId::VARIABLE
        || token == TokenId::CONSTANT
        || token == TokenId::NUMBER
        || token == TokenId::OPERATOR
        || token == TokenId::STRING
}

fn is_identifier_token(token: TokenId) -> bool {
    token == TokenId::TYPE
        || token == TokenId::FUNCTION
        || token == StandardToken::Method.id()
        || token == StandardToken::Property.id()
}

fn should_skip_context(text: &str, start: usize, end: usize) -> bool {
    let chunk_start = text[..start]
        .rfind(char::is_whitespace)
        .map_or(0, |index| index + 1);
    let chunk_end = text[end..]
        .find(char::is_whitespace)
        .map_or(text.len(), |index| end + index);
    let chunk = &text[chunk_start..chunk_end];
    chunk.contains("://")
        || chunk.contains('@')
        || chunk.contains("::")
        || chunk.contains('_')
        || chunk.contains('\\')
        || chunk.chars().any(|character| character.is_ascii_digit())
}

fn should_skip_proper_name(word: &str) -> bool {
    const COMMON_MISSPELLINGS: &[&str] = &[
        "accomodate",
        "definately",
        "occured",
        "recieve",
        "seperate",
        "teh",
    ];
    let lower = word.to_lowercase();
    if COMMON_MISSPELLINGS.contains(&lower.as_str()) {
        return false;
    }
    let mut characters = word.chars();
    let title_case = characters.next().is_some_and(char::is_uppercase)
        && characters
            .clone()
            .all(|character| !character.is_uppercase());
    let internal_uppercase = characters.any(char::is_uppercase);
    title_case || internal_uppercase || (word.len() > 1 && word.chars().all(char::is_uppercase))
}

fn identifier_words(start: usize, word: &str) -> Vec<(usize, usize, &str)> {
    let chars: Vec<(usize, char)> = word.char_indices().collect();
    let mut boundaries = vec![0];
    for index in 1..chars.len() {
        let (_, previous) = chars[index - 1];
        let (offset, current) = chars[index];
        let next = chars.get(index + 1).map(|(_, character)| *character);
        if current.is_uppercase()
            && (previous.is_lowercase() || next.is_some_and(char::is_lowercase))
        {
            boundaries.push(offset);
        }
    }
    boundaries.push(word.len());
    boundaries
        .windows(2)
        .filter_map(|bounds| {
            let part = word.get(bounds[0]..bounds[1])?;
            (!part.is_empty()).then_some((start + bounds[0], start + bounds[1], part))
        })
        .collect()
}

fn words(text: &str) -> Vec<(usize, usize, &str)> {
    let mut words = Vec::new();
    let mut start = None;
    for (index, character) in text.char_indices() {
        let word_character = character.is_alphabetic()
            || (character == '\''
                && start.is_some()
                && text[index + character.len_utf8()..]
                    .chars()
                    .next()
                    .is_some_and(char::is_alphabetic));
        match (start, word_character) {
            (None, true) => start = Some(index),
            (Some(word_start), false) => {
                words.push((word_start, index, &text[word_start..index]));
                start = None;
            },
            _ => {},
        }
    }
    if let Some(start) = start {
        words.push((start, text.len(), &text[start..]));
    }
    words
}

/// Byte offset → [`LineCol`], built once per checked text.
pub(crate) struct LineIndex<'a> {
    text: &'a str,
    starts: Vec<usize>,
}

impl<'a> LineIndex<'a> {
    pub(crate) fn new(text: &'a str) -> Self {
        let mut starts = vec![0];
        starts.extend(
            text.match_indices('\n')
                .map(|(index, _)| index.saturating_add(1)),
        );
        Self { text, starts }
    }

    pub(crate) fn position(&self, byte: usize) -> LineCol {
        let line = self.starts.partition_point(|start| *start <= byte) - 1;
        let column = self.starts.get(line).map_or(0, |start| {
            u32::try_from(self.text[*start..byte].chars().count()).unwrap_or(u32::MAX)
        });
        LineCol::new(line as u32, column)
    }
}

#[cfg(test)]
mod tests {
    use std::path::Path;

    use karet_syntax::LayeredHighlighter;
    use karet_treesitter::LayeredParser;
    use karet_treesitter::language_id_from_path;

    use super::*;

    const AFF: &str = "SET UTF-8\nSFX S Y 1\nSFX S 0 s .\n";
    const DIC: &str = "10\nhello\nworld/S\nthis\nis\na\ncomment\nreceive\ntext\ntype\nname\n";

    fn dictionary() -> Option<Dictionary> {
        Dictionary::new(AFF, DIC).ok()
    }

    /// A checked text plus the pieces `SpellInput` borrows, kept alive together.
    struct Fixture {
        text: String,
        language: Option<&'static str>,
        language_selector: Option<&'static str>,
        highlights: Highlights,
        syntax_error_lines: Vec<(u32, u32)>,
        settings: Spellcheck,
        suggest: bool,
    }

    impl Fixture {
        fn input(&self) -> SpellInput<'_> {
            SpellInput {
                text: &self.text,
                language: self.language,
                language_selector: self.language_selector,
                spelling_language: SpellingLanguage::EnglishUnitedStates,
                highlights: &self.highlights,
                syntax_error_lines: &self.syntax_error_lines,
                settings: &self.settings,
                suggest: self.suggest,
            }
        }

        fn check(&self, dictionary: &Dictionary) -> Vec<Diagnostic> {
            check(&self.input(), dictionary)
        }
    }

    fn fixture(text: &str, language: &'static str) -> Fixture {
        let settings = Spellcheck {
            enabled: true,
            ..Spellcheck::default()
        };
        let path = match language {
            "Markdown" => Path::new("notes.md"),
            "JavaScript" => Path::new("source.js"),
            "TypeScript" => Path::new("source.ts"),
            "Go" => Path::new("source.go"),
            _ => Path::new("source.rs"),
        };
        let highlights = language_id_from_path(path)
            .and_then(|language| {
                let mut parser = LayeredParser::new();
                parser.parse(language, text).ok()
            })
            .map_or_else(Highlights::default, |tree| {
                LayeredHighlighter::new().highlight(&tree, text)
            });
        Fixture {
            text: text.to_owned(),
            language: Some(language),
            language_selector: karet_filetype::file_type_for_path(path).config_selector(),
            highlights,
            syntax_error_lines: Vec::new(),
            settings,
            suggest: true,
        }
    }

    #[test]
    fn markdown_checks_prose_but_skips_code_and_links() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let text = "hello wrld `mistke` https://example.test/badd\n";
        let diagnostics = fixture(text, "Markdown").check(&dictionary);
        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("wrld"));
    }

    #[test]
    fn source_checks_comments_only_and_respects_custom_words() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let text = "let misspeled = 1; // this commment is Karet\n";
        let mut request = fixture(text, "Rust");
        request.settings.words.push("commment".to_owned());
        assert!(request.check(&dictionary).is_empty());
    }

    #[test]
    fn bundled_software_terms_apply_independently_of_language_dictionary() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let diagnostics = fixture("async breakpoint commment\n", "Markdown").check(&dictionary);

        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("commment"));
        assert!(software_terms().contains("rust"));
    }

    #[test]
    fn optional_string_and_identifier_scopes_are_independent() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let text = "fn misspeled() { let value = \"wrld\"; }\n";
        let mut request = fixture(text, "Rust");
        assert!(request.check(&dictionary).is_empty());

        request.settings.strings = true;
        let strings = request.check(&dictionary);
        assert_eq!(strings.len(), 1);
        assert!(strings[0].message.contains("wrld"));

        request.settings.strings = false;
        request.settings.identifiers = true;
        let identifiers = request.check(&dictionary);
        assert_eq!(identifiers.len(), 1);
        assert!(identifiers[0].message.contains("misspeled"));

        request.syntax_error_lines = vec![(0, 0)];
        assert!(
            request.check(&dictionary).is_empty(),
            "identifier linting pauses while syntax is invalid"
        );
    }

    #[test]
    fn rust_attribute_strings_are_skipped_but_user_facing_strings_are_checked() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let text = "#[cfg(feature = \"serrde\")]\nfn f() { println!(\"wrld\"); }\n";
        let mut request = fixture(text, "Rust");
        request.settings.strings = true;

        let diagnostics = request.check(&dictionary);

        assert_eq!(diagnostics.len(), 1, "{diagnostics:?}");
        assert!(diagnostics[0].message.contains("wrld"));
    }

    #[test]
    fn identifier_scope_checks_snake_and_camel_case_components() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let text = "struct MisspeledType;\nfn misspeled_name() {}\n";
        let mut request = fixture(text, "Rust");
        request.settings.identifiers = true;

        let diagnostics = request.check(&dictionary);

        assert_eq!(diagnostics.len(), 2, "{diagnostics:?}");
        assert_eq!(
            diagnostics
                .iter()
                .map(|diagnostic| diagnostic.range)
                .collect::<Vec<_>>(),
            vec![
                Range {
                    start: LineCol::new(0, 7),
                    end: LineCol::new(0, 16),
                },
                Range {
                    start: LineCol::new(1, 3),
                    end: LineCol::new(1, 12),
                },
            ]
        );
    }

    #[test]
    fn prose_scope_can_be_disabled_without_affecting_the_feature_toggle() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let mut request = fixture("hello wrld\n", "Markdown");
        request.settings.documents = false;
        assert!(request.check(&dictionary).is_empty());
    }

    #[test]
    fn common_misspelling_is_not_hidden_as_a_proper_name() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        let diagnostics = fixture("Recieve this\n", "Markdown").check(&dictionary);
        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("Recieve"));
    }

    #[test]
    fn suggestions_are_opt_in_so_a_bulk_scan_can_skip_the_suggester() {
        let Some(dictionary) = dictionary() else {
            return;
        };
        // "hell" is one edit from the dictionary's "hello", so the suggester has
        // something to offer — and must stay silent when `suggest` is off.
        let mut request = fixture("hell thsi\n", "Markdown");
        let suggested = request.check(&dictionary);
        assert!(
            suggested.iter().any(|d| d.message.contains("; try ")),
            "{suggested:?}"
        );

        request.suggest = false;
        let bare = request.check(&dictionary);
        assert_eq!(
            bare.iter().map(|d| d.range).collect::<Vec<_>>(),
            suggested.iter().map(|d| d.range).collect::<Vec<_>>(),
            "skipping suggestions must not change which words are flagged"
        );
        assert!(
            bare.iter().all(|d| !d.message.contains("; try ")),
            "{bare:?}"
        );
    }

    #[test]
    fn word_tokenizer_keeps_internal_apostrophes_and_utf8_offsets() {
        assert_eq!(
            words("élan isn't end"),
            vec![(0, 5, "élan"), (6, 11, "isn't"), (12, 15, "end")]
        );
    }

    #[test]
    fn prose_document_names_match_the_filetype_registry() {
        for language in [
            "Markdown",
            "Plain Text",
            "reStructuredText",
            "AsciiDoc",
            "TeX",
        ] {
            assert!(is_prose_document(Some(language)), "{language}");
        }
        assert!(!is_prose_document(Some("Rust")));
        assert!(!is_prose_document(None));
    }
}
