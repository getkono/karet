...
fn closing_a_loading_commit_cancels_it_and_late_results_stay_closed() {
    ...
    assert!(!app.all_tabs().any(|tab| matches!(
        tab.kind,
        TabKind::Commit { .. } | TabKind::CommitLoading { .. }
    )));
}
...
fn late_commit_ready_after_closing_cannot_resurrect_the_tab() {
    ...
    app.on_backend_event(
        ...
        SessionEvent::CommitReady {
            detail: Box::new(commit_detail("aaaaaaa111", "first")),
            changes: (0..64)
                .map(|index| prepared_change(&format!("src/file-{index}.rs"), StatusKind::Modified))
                .collect(),
        },
    );
...
fn commit_editor_selection_replaces_and_deletes_words() {
    ...
    assert_eq!(
        app.commit_input.edit.selected_text(&app.commit_input.text),
        Some("text")
    );
    app.commit_edit(KeyEvent::new(KeyCode::Char('!'), KeyModifiers::NONE));
...
fn conflicted_change_opens_editable_merged_view_with_read_only_sides() {
    ...
    assert_eq!(
        app.tabs.len(),
        1,
        "the existing editor is upgraded in place"
    );
