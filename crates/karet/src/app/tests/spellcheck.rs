...
fn double_clicking_a_spell_warning_opens_replacements_and_add_word() {
    ...
    let targets: Vec<DictionaryTarget> =
        app.context_menu
            .as_ref()
            .map(|menu| {
                menu.entries
                    ...
                    .filter_map(|entry| match entry.action {
                        ...
                        ContextMenuAction::Command(_)
                        | ContextMenuAction::ReplaceSpelling { .. } => None,
                    })
                    .collect()
            })
            .unwrap_or_default();
...
fn accepting_a_spelling_suggestion_is_one_atomic_edit() {
    ...
    let changes = backend
        ...
        .map(|sent| {
            sent.iter()
                .filter(|(_, command)| matches!(command, SessionCommand::ApplyChange { .. }))
                .count()
        })
        .unwrap_or_default();
    assert_eq!(changes, 1);
...
fn debounced_spell_warning_replaces_pending_lsp_with_spelling_completions() {
    ...
    app.on_backend_event(
        ...
        SessionEvent::DiagnosticsPublished {
            doc: DocumentId(9),
            diagnostics: vec![spell_diagnostic("Unknown word “wrod”; try word, wood")],
        },
    );
